...
import { Bell, BellRing, Search, Mail, Newspaper, Sparkles, ArrowUpRight, Shield, ShieldCheck, Copy, ClipboardCheck, TriangleAlert, Layers, Settings2, Send, CheckCircle2, Globe, Database, RefreshCcw } from "lucide-react";
...
function useMockPoller(enabled: boolean, push: (i: any)=>void){
  useEffect(()=>{
    if(!enabled) return; let i=0; const t=setInterval(()=>{ const b=seedInsights[i%seedInsights.length]; push({...b, id:`${b.id}-${Date.now()}`, date:new Date().toISOString()}); i++; }, 9000); return ()=>clearInterval(t);
  },[enabled,push]);
}
...
export default function App(){
  ...
  useMockPoller(settings.autoIngest, (i)=>{
    setInsights(prev=>[i,...prev]);
    if(settings.notifications && ("Notification" in window) && Notification.permission==="granted"){
      new Notification(`New insight: ${i.title}`, {body:`${i.competitor} • ${i.sourceName}`});
    }
  });
  ...
  useEffect(()=>{ setLoading(true); const t=setTimeout(()=>setLoading(false), 600); return ()=>clearTimeout(t);},[]);
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(60rem_60rem_at_80%_-10%,#E6F0FF_0%,transparent_50%),linear-gradient(#FFFFFF,#F8FAFC)] text-slate-800">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-2">
                        ...
                        {i.tags.map(t=>(<span key={t} className="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs text-slate-700">#{t}</span>))}
        ...
        <RightRail curated={curated} markdown={markdown} copied={copied} copy={copy} sending={sending} setSending={setSending} />
      </main>

      <footer className="border-t border-slate-200/70 py-6">
        <div className="mx-auto max-w-7xl px-4 text-xs text-slate-500">
          Designed for confidence. Replace mock ingestion with your crawler/GitHub Action when deploying.
        </div>
      </footer>
    </div>
  );
            ...
            {curated.map(c=> (
              <li key={c.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  {c.tags.map(t=> (<span key={t} className="rounded-full bg-[#ECF3FF] px-2.5 py-0.5 text-xs font-medium text-[#153A66] ring-1 ring-[#C8DAFF]">#{t}</span>))}
                </div>
              </li>
            ))}
          </ul>
